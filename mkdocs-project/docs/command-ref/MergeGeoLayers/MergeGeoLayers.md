...
| --------------|-----------------|----------------- |
|`GeoLayerIDs` <br>  **_required_**| A comma-separated list of the identifiers of the GeoLayers to merge. <br><br> *GeoLayerID1, GeoLayerID2, ...* <br> <br>Can also be `*` where all GeoLayers registered within the GeoProcessor are merged.| None - must be specified. |
...
* If an attribute from an `INPUT` GeoLayer is not listed as a `KEY` in the attribute map, the attribute name will be retained in the `OUTPUT MERGED` GeoLayer.
